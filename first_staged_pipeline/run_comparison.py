...
def compute_doc_metrics(
    ...
    llm_chars = llm.prompt_chars
    staged_chars = staged.prompt_chars
    token_ratio = round(staged_chars / llm_chars, 2) if (llm_chars and staged_chars) else None
    ...
    return {
        ...
        "tokens_llm_chars": llm_chars,
        "tokens_llm_est": llm.prompt_tokens_est,
        "tokens_staged_chars": staged_chars,
        "tokens_staged_est": staged.prompt_tokens_est,
        "tokens_staged_vs_llm_ratio": token_ratio,
...
def build_summary_df(doc_metrics: list[dict]):
    ...
    cols = [
        "file",
        "fill_rate_llm", "fill_rate_staged",
        "field_match_rate", "substantive_match_rate", "avg_semantic_similarity",
        "tokens_llm_chars", "tokens_llm_est",
        "tokens_staged_chars", "tokens_staged_est",
        "tokens_staged_vs_llm_ratio",
    ]
    return pd.DataFrame([{c: m.get(c) for c in cols} for m in doc_metrics])
...
def main():
    ...
    for fname, llm_e, staged_e in pairs:
        ...
        llm_tok  = f"{m['tokens_llm_est']:,}"   if m.get('tokens_llm_est')    else "n/a"
        stg_tok  = f"{m['tokens_staged_est']:,}" if m.get('tokens_staged_est') else "n/a"
        ratio    = f"{m['tokens_staged_vs_llm_ratio']}x" if m.get('tokens_staged_vs_llm_ratio') else "n/a"
        print(
            ...
            f"tokens LLM≈{llm_tok} staged≈{stg_tok} (ratio {ratio})"
        )
