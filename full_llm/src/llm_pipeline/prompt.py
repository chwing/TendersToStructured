from ..extractor import ALL_FIELDS
...
def make_user_prompt(document_text: str, fields: list[str] | None = None) -> str:
    """Build a user prompt asking the model to extract only the given fields."""
    if fields is None:
        fields = ALL_FIELDS
    fields_line = ", ".join(fields)
    return (
        "Extract the following fields from the tender document below.\n\n"
        "Return a JSON object where each key maps to "
        "{\"value\": <extracted value or null>, \"confidence\": \"high\"|\"medium\"|\"low\"}:\n\n"
        f"{fields_line}\n\n"
        "---\nDOCUMENT:\n"
        f"{document_text}\n"
